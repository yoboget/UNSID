...
def main() -> None:
    ...
    if work_type == 'train':
        ...
        wandb.init(project=f'Dirisid_{dataset}', config=config, mode=args.wandb)
        dataloader = get_dataset(config)
        trainer = Trainer(dataloader, config)
        trainer.train()

    elif  work_type == 'train_classifier':
        wandb.init(project=f'Dirisid_{dataset}_classifer', config=config, mode=args.wandb)
        dataloader = get_dataset(config)
        trainer = Trainer(dataloader, config)
        trainer.train()
        ...
        if dataset in ['qm9', 'qm9_cc', 'qm9H', 'zinc']:
            print(f'{latex_format["valid"]} &  {latex_format["fcd"]} & {latex_format["nspdk"]} & {latex_format["unique"]} & {latex_format["novel"]} \\')
        else:
            if 'orbit' in latex_format.keys():
                latex_format["valid"], latex_format['novel'] = 0, 0
                print(f'{latex_format["valid"]} &  {latex_format["degree"]} & {latex_format["cluster"]} & {latex_format["orbit"]}& {latex_format["spectral"]} & {latex_format["novel"]} \\')
            else:
                print(
