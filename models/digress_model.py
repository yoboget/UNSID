...
from models.digress_layers import Xtoy, Etoy, masked_softmax
...
class GraphTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.out_dim_X = nnf_out
        self.out_dim_E = nef_out
        self.out_dim_y = 0
        nhfe = int(nhf/config.model.edge_node_ratio)
        y_in = 1

        self.mlp_in_X = nn.Sequential(
            nn.Linear(nnf_in, nhf),
            act_fn_in,
            nn.Linear(nhf, nhf),
            act_fn_in,
        )
        ...
        self.mlp_out_X = nn.Sequential(
            nn.Linear(nhf, nhf),
            act_fn_out,
            nn.Linear(nhf, nnf_out),
        )

        self.mlp_out_E = nn.Sequential(
            nn.Linear(nhfe, nhfe),
            act_fn_out,
            nn.Linear(nhfe, nef_out),
        )
    ...
    def forward(self, X, E, node_mask, t):
        bs, n = X.shape[0], X.shape[1]
        y = t  # extract time step from the first node
        ...
        x_mask = node_mask.unsqueeze(-1)  # bs, n, 1
        X = X * x_mask
        E = new_E * x_mask.unsqueeze(2) * x_mask.unsqueeze(1)
