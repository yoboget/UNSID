...
def eval_planar(graph_ref_list, graph_pred_list):
    ...
    uniq, uniq_non_iso, uniq_non_iso_val = eval_fraction_unique_non_isomorphic_valid(graph_pred_list, graph_ref_list,
                                                                                     validity_func)
    ...
    for method in methods:
        if 'eval' in method:  # sbm, planar
            ...
            if method in ['degree', 'cluster', 'orbit', 'spectral']:  # MMD requires kernel
                result = METHOD_NAME_TO_FUNC[method](graph_ref_list, graph_pred_list, KERNEL='emd')
            else:
