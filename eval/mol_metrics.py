...
def mol_metric(gen_mols, dataset, num_no_correct, test_metrics=False):
    ...
    metrics['valid'] = num_no_correct/n
    gen_valid = remove_invalid(gen_smiles, canonize=True)
    ...
    if len(gen_valid) > 0:
        ...
        metrics['unique'] = 0
    if test_metrics:
        if dataset == 'qm9_conditional' or dataset == 'qm9_cc':
            dataset = 'qm9'
        ...
        with open(f'./data/{dataset}/test_nx_{dataset}.pkl', 'rb') as f:
            test_graph_list = pickle.load(f)
            random.Random(42).shuffle(test_graph_list)

        # for i, smi in enumerate(test_smiles[:20]):
        #     mol = Chem.MolFromSmiles(smi)
        #     Chem.AddHs(mol)
        #     Draw.MolToFile(mol, f'./misc/plots/qm9H_test_{i}.png')
        ...
        metrics['fcd'] = FCD(n_jobs=0, device=device)(ref=test_smiles, gen=gen_valid)
        kwargs = {'n_jobs': 1, 'device': device, 'batch_size': 100}
        ...
        # metrics['FCD/Test'] = FCD(**kwargs)(gen=gen_smiles, pref=ptest['FCD'])
...
def mols_to_smiles(mols, explicit_h=False):
    return [Chem.MolToSmiles(mol, allHsExplicit=explicit_h) for mol in mols]
...
def load_smiles(dataset='qm9'):
    if dataset == 'qm9H':
        ...
        df = pd.read_csv(f'./data/{dataset}/smiles_{dataset}.csv')

        with open(f'./data/{dataset}/test_idx_{dataset}.json') as f:
...
def get_molecular_properties(mols):
    ...
    for mol in mols:
        # Chem.SanitizeMol(mol)
        try:
            qed_val = qed(mol) #- min_qed
            # qed = qed / range_qed
        except Exception as e:
            print("Error computing QED:", e)
            qed_val = 0
        ...
        values = torch.cat((values, torch.tensor([weight, logP, qed_val]).unsqueeze(0)), dim=0)
    return values
...
def compute_intermediate_statistics(smiles, n_jobs=1, device='cpu',
    ...
    statistics['FCD'] = FCD(**kwargs_fcd).precalc(smiles)
